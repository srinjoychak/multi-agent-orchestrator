 ...
 *
 * Key differences from the old gemini-ask.js:
 *   - `agy` has no --output-format json flag; output is plain markdown text.
 *   - `agy` has no API-key env var (OAuth only). Isolation is done by
 *     redirecting $HOME for the subprocess (agy has no dedicated
 *     config-dir override env var), copying just the OAuth token in.
 *   - Approval is via --dangerously-skip-permissions instead of
 *     --approval-mode yolo.
 */
...
async function isolatedAgyAuth() {
  ...
  await chmod(tempHome, 0o700); // Prevent other users from reading temp credentials
